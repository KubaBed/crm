...
This library provides convenient access to the CRM API from TypeScript or JavaScript.
...
| --- | --- | --- | --- |
| `cookie` | `string \| provider` | - | Credential for the cookie scheme. Defaults to COOKIE. |
| `apiKey` | `string \| provider` | - | Credential for the apiKey scheme. Defaults to API_KEY. |

Declared schemes:

- `cookie` API key in cookie `crm.session_token`
...
Documented error statuses: `400`, `401`, `403`, `404`, `500`, `503`.
...
| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `cookie` | `string \| AuthTokenProvider` | `process.env["COOKIE"]` | Credential for the cookie scheme. |
| `apiKey` | `string \| AuthTokenProvider` | `process.env["API_KEY"]` | Credential for the apiKey scheme. |
| `baseURL` | `string \| null` | `process.env["TRYCOMP_BASE_URL"]` | Override the default API base URL. Pass `null` when selecting a configured environment. |
...
| `logLevel` | `"off" \| "error" \| "warn" \| "info" \| "debug" \| null` | `process.env["TRYCOMP_LOG"]` | Controls request and retry debug logging. |
